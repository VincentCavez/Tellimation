...
**Shortcut CH → Silhouette** : Si le macro sélectionné est `"CH"` et qu'il existe des entités non nommées (pas dans `session.character_names`), on force `I2_silhouette` (l'entité devient une silhouette plate qui demande « qui est-ce ? ») sans appeler Gemini.
...
- Les **suggestion_intents filtrés** : uniquement les animations dont `misl_elements` contient le code sélectionné. Ex pour `"S"` : `S1_peek`, `S2_stamp`, `T2_timelapse`
...
## Note — remplacement d'animations (août 2026)

Trois animations de l'étude 1 ont été remplacées à IDs constants (`I2`, `C3`, `S1`) :

| ID | Étude 1 | Actuel | Visuel |
|----|---------|--------|--------|
| I2 | Nametag (`nametag`) | Silhouette (`silhouette`) | L'entité devient une silhouette plate, pulse, puis révèle ses couleurs avec un flash |
| C3 | Ghost Outline (`ghost_outline`) | Missing Piece (`missing_piece`) | Un trou en pièce de puzzle s'ouvre là où l'entité absente irait ; la pièce, portant sa silhouette, flotte au bord du cadre |
| S1 | Reveal (`reveal`) | Peek (`peek`) | L'occulteur se soulève comme un volet charnière (scale horizontal + assombrissement), exposant les pixels d'arrière-plan ; halo optionnel sur `entityPrefixB` |

- Sémantique de sélection inchangée (mêmes intents, sauf S1 : `misl_elements` réduit à `["S"]`).
- `missing_piece` reçoit `pieceImageUrl` injecté côté serveur (`_resolve_piece_image_url` dans
  `src/ui/app.py`) : le cut-out de l'entité absente est cherché dans les autres scènes de la story.
- Les anciennes implémentations vivent dans `src/ui/static/animation_templates_legacy.js`,
  chargé uniquement par `simulation.html` et `scripts/study_gen/video_ui.html` pour régénérer
  les stimuli de l'étude 1.
- Escalade : `load_animation_params()` pousse les int/float vers le max du range en cas de
  non-résolution — tous les paramètres des trois animations sont orientés « max = plus saillant »
  (d'où `openness` plutôt qu'un `flapMin` inversé).
